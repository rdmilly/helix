...
def _entity_to_dict(row) -> Dict:
    ...
    if "attributes_json" in d:
        try: d["attributes"] = pg_sync.dejson(d["attributes_json"])
        except: d["attributes"] = {}
        del d["attributes_json"]
    if "meta" in d:
        try: d["meta"] = pg_sync.dejson(d["meta"])
        except: d["meta"] = {}
    return d
